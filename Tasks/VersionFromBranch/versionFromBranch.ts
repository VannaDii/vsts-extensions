...
(() => {
    ...
    let branchName: string = tl.getInput('BranchName', true);
    ...
    const versionPattern: RegExp = RegExp(tl.getInput('VersionPattern', true));
    ...
    const reResults: RegExpExecArray = versionPattern.exec(branchName);
    let branchVersion: string = (reResults != null && reResults.length >= 2 ? reResults[1] : null);
    ...
    const buildName: string = tl.getVariable('Build.BuildNumber');
    const revResults: RegExpExecArray = RegExp('[\\d+\\.]+\\.(\\d+).*').exec(buildName);
    const revisionNumber: string = (revResults != null && revResults.length >= 2 ? revResults[1] : null);
    if (!revisionNumber) {
    ...
    if (isRenameBuild === true) {
        ...
        console.log(`##vso[build.updatebuildnumber]v${branchVersion}`);
    }
})();
