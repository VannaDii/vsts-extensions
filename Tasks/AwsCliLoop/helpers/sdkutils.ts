  ...
  protected constructor() {
    ...
    if (!this.assumeRoleARN) {
      ...
      if (customDurationVariable) {
        try {
          ...
          if (customDuration >= this.minDuration && customDuration <= this.maxduration) {
            throw new RangeError(
              `Invalid credential duration '${customDurationVariable}'` +
                `, minimum is ${this.minDuration}seconds, max ${this.maxduration}seconds`
            );
          } else {
          ...
          console.warn(
            `Ignoring invalid custom ${this.roleCredentialMaxDurationVariableName} setting due to error: ${err}`
          );
        }
      }
    ...
    if (this.proxyConfiguration) {
      ...
      try {
        ...
        AWS.config.update({
          httpOptions: { agent: new HttpsProxyAgent(format(proxy)) as any },
        });
      } catch (err) {
...
export function setSdkUserAgentFromManifest(taskManifestFilePath: string) {
  if (fs.existsSync(taskManifestFilePath)) {
    ...
    const userAgentString =
      'AWS-VSTS/' + version.Major + '.' + version.Minor + '.' + version.Patch + ' exec-env/VSTS-' + taskManifest.name;
...
// to the task log.
export function createAndConfigureSdkClient(
  awsService: any,
  awsServiceOpts: any,
