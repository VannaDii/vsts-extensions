...
async function getBuildInfo(
  ...
  maxBuilds: number = 25,
  type: string = 'Lines'
): Promise<IBuildCodeCoverageStats[]> {
...
async function loadWithSettings(widgetSettings: any): Promise<WidgetContracts.WidgetStatus> {
  try {
    ...
    return getBuildInfo(projectId, buildDefId, undefined, measurementName)
      .then((stats) => {
        ...
        chartContainer.show().css('width', 300).css('height', 82).css('position', 'relative').css('margin-bottom', 8);
        ...
        if (showMeasurementName) {
          $('#measurement-label')
            .show()
            .text(' of ' + measurementName.toLocaleLowerCase());
        } else {
          $('#measurement-label').hide();
        }
          ...
          bar
            ...
            .attr(
              ...
              stat.definitionName +
                ...
                stat.number +
                ' @ ' +
                stat.values.percent.toFixed(decimalPlaces) +
                '% of ' +
                measurementName
            )
            .click((e) => {
  ...
  VSS.register('CodeCoverageChartWidget', function () {
    return {
      load: function (widgetSettings: any) {
        return loadWithSettings(widgetSettings);
      },
